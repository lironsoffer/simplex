...
\title{\textbf{Belief State Geometry in Non-Ergodic Transformers}\\
\large A Mess3 Mixture Experiment}
\author{}
\date{March 2026}
...
from non-ergodic pretraining.
The model's in-context performance is coupled to its ability to identify which
component is generating the current sequence.
The synchronisation horizon
$N^* = 1/\min_{k \neq k'} D_\mathrm{KL}(P_k \| P_{k'})$
is the number of tokens needed to commit to one component.
By choosing parameters with $N^* < 16$ (context window), this commitment
happens within a single sequence.
...
\begin{tabular}{lcc}
\toprule
Layer & CEV Dimensionality (95\%) & Predicted \\
\midrule
0 & 6 & 8 \\
1 & 5 & 8 \\
\bottomrule
\end{tabular}
\caption{PCA dimensionality (number of components explaining $\geq 95\%$
variance) per layer.}
\label{tab:pca}
\end{table}

The model uses 5--6 effective dimensions rather than the predicted 8
(prediction P1 partially confirmed).
One explanation is that component C has a near-degenerate attractor
($\alpha=0.55$, only $\sim$9 reachable belief states), collapsing the
$\eta_C$ subspace from 2D toward 0D.
A second explanation is superposition: the model may pack the 8 theoretical
dimensions into fewer effective directions.
...
The centre shows the meta-belief $\pi$ simplex; each vertex hosts the
MSP fractal attractor of the corresponding component.
The joint belief space consists of three disconnected fractals---one
per component---reflecting the non-ergodic block structure.}
\label{fig:joint}
\end{figure}
...
\newblock \emph{NeurIPS 2024}. arXiv:2405.15943.
...
\newblock \emph{ICML 2025}. arXiv:2602.02385.
